...
const getState = new httpRequest(
	"https://nft-collector.herokuapp.com/users/change-request"
);
getState.getRequest().then((settings) => {
...
async function populateGallery() {
	...
	const getPictures = fetch("https://nft-collector.herokuapp.com/users/pics")
		.then((data) => data.json())
		.then((data) => {
			const pics = data;
			pics.map((pic) => {
...
function patch(user, url) {
	const getUserInfo = new httpRequest(
		`https://nft-collector.herokuapp.com/users/${user[0].username}`
	);
...
function changeLogin(e) {
	...
	if (
		e.target.id === "change-parameter" &&
		!e.target.classList.contains("patch-username") &&
		!e.target.classList.contains("delete-account")
	) {
		e.preventDefault();
...
function checkIfValid(username, password) {
	...
	getUser().then((user) => {
		//if the inputs match
		const checkInfo = new httpRequest(
			"https://nft-collector.herokuapp.com/users",
			username,
			null,
			password
		);
		checkInfo.loginRequest(true).then((resp) => {
			console.log(resp);
...
function userAuthenticated() {
	getState.getRequest().then((update) => {
		//if the user wanted to change their password show the change password state
		if (update.password) {
			formDiv.style.display = "none";
			formDiv.classList.add("hidden");
		...
		if (passwordInput.value !== "" && confirmPasswordInput.value !== "") {
			if (passwordInput.value !== confirmPasswordInput.value) {
				...
				//if the new password meets the criteria then send the patch request to the server with the new password
					...
					getUser().then((user) => {
						const setPassword = new httpRequest(
							`https://nft-collector.herokuapp.com/users/${user[0].username}`
						);
...
function checkIfUsernameValid(username) {
	...
	if (hasNumber) {
		...
		if (username.length < 6) {
			...
			getLoggedInUser.getRequest().then((users) => {
				...
				//send a patch request with the new username
					...
					getUser().then((user) => {
						const setUsername = new httpRequest(
							`https://nft-collector.herokuapp.com/users/${user[0].username}`
						);
	...
	} else [createErrorMessage("New username must have atleast one number")];
}
	...
	if (e.target.classList.contains("delete-account")) {
		...
		if (!confirmDelete.checked && !dontDelete.checked) {
			...
			if (dontDelete.checked) {
				...
				getUser().then((user) => {
					const deleteAccount = new httpRequest(
						`https://nft-collector.herokuapp.com/users/${user[0].username}`
					);
