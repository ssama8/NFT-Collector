...
const getOrderedNFT = new httpRequest(
...
async function getCorrectNFT() {
	...
	getOrderedNFT.getRequest().then((data) => {
		nftSpot.forEach((img) => {
			img.src = data.url;
		});
...
async function getUser(img) {
	...
	getLoggedInUser.getRequest().then((data) => {
		...
		const postLoggedInUser = new httpRequest(
			"https://nft-collector.herokuapp.com/users",
			targetUser[0].username,
			img,
			targetUser[0].password,
			parseInt(counterValue.textContent)
		);
