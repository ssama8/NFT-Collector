...
function togglePasswordVisibility(e) {
	if (
		e.target.id === "password-toggle" ||
		e.target.id === "inner-password-toggle"
	) {
...
function validateInputs(e) {
	...
	if (userName !== "" && password !== "" && confirmPassword !== "") {
		...
		getRequest(userName).then((userExists) => {
			//If the user exists than create error that the username can't be used.
			if (userExists) {
				createErrorMessage("Username has already been taken");
			} else {
...
async function postUser(image) {
	//sends post request
	const rawResponse = await fetch("https://nft-collector.herokuapp.com/users", {
		method: "POST",
		headers: {
